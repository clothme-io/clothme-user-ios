...
class Router<EndPoint: EndPointType>: NetworkRouter {
    ...
    fileprivate func buildRequest(from route: EndPoint) throws -> URLRequest {
        ...
        do {
            ...
            case .requestParametersAndHeaders(bodyParameters: let bodyParameters, urlParameters: let urlParameters, additionHeaders: let additionalHeaders):
                self.addAddtionalHeaders(additionalHeaders, request: &request)
                try self.configureParameters(bodyParameters: bodyParameters, urlParameters: urlParameters, request: &request)
            }
            return request
        } catch {
    ...
    fileprivate func addAddtionalHeaders(_ additionalHeaders: HTTPHeaders?, request: inout URLRequest) {
