...
class AccountDomainMapper {
    ...
    static func toApplicationDomain(account: Account) -> AccountApplicationModel {
        return AccountApplicationModel(ownerId: account.accountOwnerId.value().toIdString(), accountUsers: accountList(account), numberOfAccount: account.numberOfAccount)
    }
    ...
    private static func accountList(_ account: Account) -> [AccountUserApplicationModel] {
        ...
        while account.accounts.count >= index {
            let users = AccountUserApplicationModel(accountId: account.accounts[index].accountId.value().toIdString(), userId: account.accounts[index].Id.eId.toIdString(), firstName: account.accounts[index].firstName.value, lastName: account.accounts[index].lastName?.value ?? "", dateOfBirth: account.accounts[index].dateOfBirth?.value ?? "", gender: account.accounts[index].gender.value ?? "", phoneNumber: getPhoneNumberModel(account.accounts[index]), relationship: account.accounts[index].relationShip.type, dateAdded: account.accounts[index].dataAdded.value, shippingAddress: getShippingAddressModel(account.accounts[index]), brandId: getBrandIdModel(account.accounts[index]))
